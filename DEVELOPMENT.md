...
- 서버형 EFT 레이드의 PMC·스캐브는 같은 매칭 세대의 ProfileId 동일·정확한 `+1` 관계와 파티의 실제 캐릭터 Side를 제한적으로 사용합니다. 로그의 `PvpSeasonN` 번호는 클라이언트 버전 매핑보다 우선합니다. `/client/match/local/start`가 있는 로컬 PvE는 솔로만 확정하며, 현재 확인된 로컬 로그에는 역할 직접 근거가 없으므로 PMC·스캐브를 추측하지 않습니다.
...
- 실게임 Statistics 행 자체가 없으면 `로그없음`, 행은 있으나 수신 표본이 0건이면 RTT를 `표본부족`으로 구분합니다. 이때 로그의 패킷손실 값이 유효하면 `(참고)`를 붙여 보존하며, `sent`·`received`의 차이로 패킷손실을 새로 계산하지 않습니다.
- 서버차단현황을 열 때 화면 기간·이전 조회 캐시와 별도로 전체 로그에서 최신 레이드 최대 100개를 다시 확정합니다. 해당 IP의 접속 자체가 없으면 `최근 기록 없음`, 접속은 있으나 Statistics 행이 없으면 `로그없음`, 전체 스캔을 확정하지 못하면 `확인 불가`로 구분합니다. 실게임 RTT와 패킷손실은 IP별로 각각 가장 최근의 유효한 레이드 값을 선택하므로 두 값의 기록 시각이 다를 수 있습니다. 차단 규칙이 ICMP를 포함한 통신을 막는 동안 현재 핑은 측정하지 않으며, 측정을 위해 규칙을 자동 해제·재적용하지 않습니다.
- Windows 방화벽 차단은 PC별입니다. 파티원 모두의 접속을 막으려면 각 PC에서 같은 서버를 차단해야 하며, 앱은 차단 목록을 자동 공유·동기화하지 않습니다.
- 사용방법의 `나가기 확인`은 서버 입장이 완료되지 않은 차단 흐름을 설명합니다. 이 단계에서는 장비가 보존되며 일반 레이드 입장 후 이탈 안내로 해석하지 않습니다.

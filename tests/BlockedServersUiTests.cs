...
namespace TarkovServerReporter.Tests
{
    internal static class BlockedServersUiTests
    {
        ...
        private static void AssertCurrentPingPresentation(Assembly application)
        {
            ...
            using (var form = (Form)Activator.CreateInstance(formType))
            {
                ...
                var toolTip = (ToolTip)formType.GetField(
                    "_toolTip",
                    instanceFlags).GetValue(form);
                MethodInfo applyCurrentPing = formType.GetMethod(
                    "ApplyCurrentPingCell",
                    staticFlags);
                Assert(grid != null && refreshButton != null && toolTip != null
                        && applyCurrentPing != null,
                    "서버차단현황의 현재 핑 표시 경계를 찾지 못했습니다.");
                Assert(!grid.Columns.Contains("actualRtt")
                        && !grid.Columns.Contains("packetLoss")
                        && grid.Columns.Contains("currentPing"),
                    "서버차단현황은 제거된 실게임 지표 열 없이 현재 핑 열을 유지해야 합니다.");

                DataGridViewRow row = AddServerRow(grid, serverType, "192.0.2.10");
                applyCurrentPing.Invoke(null, new object[] { row });

                const string currentPingHelp =
                    "차단 규칙이 해당 IP의 모든 아웃바운드 통신(ICMP 포함)을 막으므로 차단 중에는 "
                    + "현재 핑을 측정할 수 없습니다. 해제 후 메인 화면에서 조회해 주세요.";
                Assert(Convert.ToString(row.Cells["currentPing"].Value)
                        == "차단 중 · 측정 불가"
                    && row.Cells["currentPing"].ToolTipText == currentPingHelp
                    && row.Cells["currentPing"].AccessibilityObject.Description
                        == currentPingHelp,
                ...
                Assert(grid.Columns["currentPing"].HeaderCell.ToolTipText.Contains(
                    ...
                    && grid.AccessibleDescription.Contains(
                        "차단 중에는 측정할 수 없습니다."),
                    "현재 핑의 측정 제약을 헤더 툴팁과 접근성 설명에서 동일하게 제공해야 합니다.");
                ...
                Assert(refreshButton.AccessibleName == "차단 규칙 목록 새로고침"
                    ...
                    "창 안 새로고침은 Windows 방화벽 차단 규칙만 다시 확인한다고 안내해야 합니다.");

                form.CreateControl();
                form.PerformLayout();
                grid.CreateControl();
                grid.PerformLayout();
                Assert(form.ClientSize.Width == 1120
                    && !grid.Controls.OfType<HScrollBar>().Any(scroll => scroll.Visible),
                    "축소된 기본 창 폭에서도 가로 스크롤이 처음부터 나타나면 안 됩니다.");
        ...
        private static DataGridViewRow AddServerRow(
            DataGridView grid,
            Type serverType,
            string ipAddress)
        {

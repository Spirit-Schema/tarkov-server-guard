# Tarkov Server Guard v0.8.7

<p align="center">
  <img src="https://raw.githubusercontent.com/Spirit-Schema/tarkov-server-guard/v0.8.7/assets/branding/tarkov-server-guard-tsg-icon-master.png" width="384" alt="Tarkov Server Guard logo">
</p>
...
- **[Download installer — recommended](https://github.com/Spirit-Schema/tarkov-server-guard/releases/download/v0.8.7/SpiritSchema.TarkovServerGuard-win-Setup.exe)**
- [Download Portable ZIP — no installation](https://github.com/Spirit-Schema/tarkov-server-guard/releases/download/v0.8.7/SpiritSchema.TarkovServerGuard-win-Portable.zip)
...
The executable is unsigned, so Windows may show an unknown-publisher warning. Use the official downloads above; [SHA-256 checksums](https://github.com/Spirit-Schema/tarkov-server-guard/releases/download/v0.8.7/SHA256SUMS.txt) are available.
...
- Fixed season identification using raid dates and season history, independently of game version. Unresolved season numbers show `PvP/S?`; confirmed numbers retain `PvP/S1` notation.
- Added log-based raid end time beside raid duration, with the date shown for overnight raids.
- Launching TSG again shows the existing window instead of starting another copy.
...
- **[설치형 다운로드 — 권장](https://github.com/Spirit-Schema/tarkov-server-guard/releases/download/v0.8.7/SpiritSchema.TarkovServerGuard-win-Setup.exe)**
- [포터블 ZIP 다운로드 — 설치 없이 사용](https://github.com/Spirit-Schema/tarkov-server-guard/releases/download/v0.8.7/SpiritSchema.TarkovServerGuard-win-Portable.zip)

Windows 10·11 / .NET Framework 4.8이 필요합니다. 포터블은 압축을 풀고 폴더 안의 파일 구성을 유지해 주세요. 두 방식 모두 사용자 확인 후 업데이트할 수 있습니다.

현재 실행 파일에는 코드 서명이 없어 Windows에서 ‘알 수 없는 게시자’ 안내가 표시될 수 있습니다. 위 공식 링크에서 받고, [SHA-256 확인 파일](https://github.com/Spirit-Schema/tarkov-server-guard/releases/download/v0.8.7/SHA256SUMS.txt)로 검증할 수 있습니다.
...
- 시즌 번호를 레이드 날짜와 시즌 운영 이력으로 판별하도록 수정했습니다. 번호가 없으면 `PvP/S?`, 확인된 번호는 기존 `PvP/S1` 축약 표기로 표시합니다.
- 작전시간 옆에 로그 기반 작전종료 시각을 표시합니다. 날짜가 바뀌면 종료 날짜도 표시합니다.
- 실행 중 다시 실행하면 중복 창 대신 기존 창을 표시합니다.
- 시작 시와 실행 중 약 6시간마다 업데이트를 확인합니다. `나중에`는 해당 버전만 선택 시점부터 24시간 보류하며, 더 새 버전은 안내합니다. 확인·다운로드 실패는 보류로 저장하지 않습니다. 자동 알림은 TSG가 활성화되고 다른 작업이 끝난 뒤 표시합니다.

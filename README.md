...
- `최근100개`, `오늘`, `7일`, `30일`, 직접 지정 기간과 EFT·Arena·데이터센터 지역 필터 지원
- 서버별 차단·해제·차단 메모, 차단 목록 내보내기·불러오기 및 로그가 없어도 사용할 수 있는 `서버차단현황` 제공
- PvP, `PvP시즌1` 같은 시즌 번호, PvE(서버), PvE(로컬)과 작전시간·서버배정·연결 결과(재접속 횟수 포함) 표시
- EFT 런처와 Arena 인게임 설정에서 확인한 선택 서버 표시
- 세로형·좁은 화면에서는 고급 서버 정보를 자동으로 숨겨 핵심 정보에 집중
- 레이드 메모·유저신고 메모를 PC에 보관·선택 삭제하고, 최근 확인 후 6시간이 지난 앱 실행 시 또는 수동으로 새 안정 버전 확인
...
3. 기간·게임·데이터센터 지역 필터를 선택한 뒤 `조회`를 누릅니다. 새로 생성되거나 변경된 최신 로그를 먼저 다시 읽고 핑·지역·차단 상태를 이어서 갱신합니다.
